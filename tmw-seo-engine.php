 ...
 * Description: Intelligence Core v5.8.10-final-model-copy-polish — Small deterministic copy-cleanup polish on top of v5.8.9. Builds on v5.8.7 (3-textarea Model Research evidence flow) and v5.8.8 (ModelCopyCleanup wired into all 7 model-content save sites). NEW in v5.8.10 (no architecture changes, no evidence-system rebuild, no AWE/Rank Math/link-verification/generation-routing changes): (A) ModelCopyCleanup drops the SECOND of two adjacent identical headings (any H2/H3/H4 levels) so e.g. "<h2>Before You Click</h2><h3>Before You Click</h3>" collapses to one heading while content beneath the dropped heading is preserved verbatim. (B) ModelCopyCleanup deletes keyword-stuffed cam-show connector sentences inside paragraphs — "This is (especially) useful when (you are) researching {model} cam show." — model-name-agnostic; surviving paragraphs trimmed; resulting empty <p> wrappers removed by a final empty-paragraph drop. (C) ModelCopyCleanup adds model-keyword heading rewrites that turn standalone "{Model} Livejasmin"/"{Model} LiveJasmin" → "LiveJasmin Access Notes", "{Model} Webcam Chat" → "Webcam Chat Notes", "{Model} Live Cam" → "Live Cam Access" — works for any model name (literal match when post title is supplied, generic 1–4 name-token fallback otherwise). (D) ModelCopyCleanup drops two redundant Official Links explanatory paragraphs ("Official platform profiles are listed here…", "Verified destinations grouped by platform family…") while NEVER touching real <a> anchors, grouped link headings, platform-family headings, or <ul>/<li> blocks. (E) ModelResearchEvidence bio activity humanizer now maps "private chat" → "private-chat availability" instead of "private-chat interaction"; final bio reads "The notes also highlight private-chat availability, live cam sessions, and genuine viewer connection." (F) Manual TOC, verified-link rendering, evidence block (between <!-- tmwseo-seed-evidence:start --> markers), and existing v5.8.8 cleanup behaviour are all preserved. Idempotent — running cleanup twice produces the same output as running it once.
 * Version: 5.8.10-final-model-copy-polish
...
defined('TMWSEO_ENGINE_VERSION') || define('TMWSEO_ENGINE_VERSION', '5.8.10-final-model-copy-polish');
