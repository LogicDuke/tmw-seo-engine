...
update_post_meta(991,'_tmwseo_category_generation_run_id','run-h');update_post_meta(991,'rank_math_focus_keyword','old focus');update_post_meta(991,'rank_math_title','Old Title');update_post_meta(991,'_tmwseo_category_active_chip_set','old active contract');$oldContent=get_post_field('post_content',991);$rf=CategoryGenerationTransaction::commit(get_post(991),'<p>h</p>','<p>h</p>',['run_id'=>'run-h','validate'=>fn($x)=>['ok'=>true],'persist_metadata'=>function($id,$html){update_post_meta($id,'rank_math_focus_keyword','partial focus');update_post_meta($id,'rank_math_title','Partial Title');update_post_meta($id,'_tmwseo_category_active_chip_set','partial active contract');throw new RuntimeException('metadata failure');}]);
...
tassert(get_post_field('post_content',991)===$oldContent&&get_post_meta(991,'rank_math_focus_keyword',true)==='old focus'&&get_post_meta(991,'rank_math_title',true)==='Old Title'&&get_post_meta(991,'_tmwseo_category_active_chip_set',true)==='old active contract','failed generation restores previous Rank Math focus-keyword and active-contract values');
delete_post_meta(991,'_tmwseo_category_active_chip_set');update_post_meta(991,'_tmwseo_category_generation_run_id','run-i');$oldAbsentContent=get_post_field('post_content',991);$ri=CategoryGenerationTransaction::commit(get_post(991),'<p>i</p>','<p>i</p>',['run_id'=>'run-i','validate'=>fn($x)=>['ok'=>true],'persist_metadata'=>function($id,$html){update_post_meta($id,'_tmwseo_category_active_chip_set','new active contract');throw new RuntimeException('metadata failure after active contract');}]);
tassert($ri['failure_code']==='metadata_finalize_failed'&&$ri['rollback_status']==='verified','rollback is verified after removing a newly-created active-contract value');
tassert(get_post_field('post_content',991)===$oldAbsentContent&&get_post_meta(991,'_tmwseo_category_active_chip_set',false)===[],'failed generation restores previous active-contract absence state');
