...
check('provider draft accepted', (bool) $out['ok'], implode('; ', array_slice((array) ($out['report']['failure_reasons'] ?? []), 0, 4)));
if ($out['ok']) {
    $den = (array) ($out['report']['metrics']['density'] ?? []);
    check('provider voice preserved through density repair', strpos((string) $out['html'], 'streaming has a texture') !== false);
    check("provider density {$den['density']}% within dynamic band", ($den['status'] ?? '') === 'within', json_encode($den));
    check('provider anchors byte-identical', strpos((string) $out['html'], '<a href="https://top-models.webcam/models/">model directory</a>') !== false);
}
