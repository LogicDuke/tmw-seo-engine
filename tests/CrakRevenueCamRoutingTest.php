...
class CrakRevenueCamRoutingTest extends TestCase {
    ...
    /**
     * Sync should fail with clear message when API key is missing.
     *
     * @return void
     */
    public function test_sync_missing_api_key_sets_error_message(): void {
        update_option(CrakRevenueCamManager::API_SETTINGS_OPTION, ['api_key' => '']);
        ...
        $this->assertSame('Missing API key.', (string)($settings['last_sync_message'] ?? ''));
        $this->assertSame('error', (string)($settings['last_sync_status'] ?? ''));
    }

    /**
     * Sync should store non-200 response diagnostics.
     *
     * @return void
     */
    public function test_sync_non_200_stores_status_and_message(): void {
        update_option(CrakRevenueCamManager::API_SETTINGS_OPTION, ['api_key' => 'secret-key']);
        $GLOBALS['_tmw_test_http_queue'] = [[
            'response' => ['code' => 503],
        ...
        $settings = get_option(CrakRevenueCamManager::API_SETTINGS_OPTION, []);
        $this->assertFalse((bool)$result['ok']);
        $this->assertSame(503, (int)($settings['last_sync_http_status'] ?? 0));
        $this->assertStringContainsString('Non-200 HTTP status code', (string)($settings['last_sync_message'] ?? ''));
    }

    /**
     * Raw offers with no cam matches should emit operator-friendly diagnostics.
     *
     * @return void
     */
    public function test_sync_zero_cam_offers_includes_diagnostic_sample_names(): void {
        update_option(CrakRevenueCamManager::API_SETTINGS_OPTION, ['api_key' => 'secret-key']);
        $GLOBALS['_tmw_test_http_queue'] = [[
            'response' => ['code' => 200],
            'headers' => ['content-type' => 'application/json'],
            'body' => '{"data":[{"id":1,"name":"Generic Offer A"},{"id":2,"name":"Generic Offer B"}]}',
        ]];

        $result = CrakRevenueCamManager::sync_offers();
        $settings = get_option(CrakRevenueCamManager::API_SETTINGS_OPTION, []);
        $this->assertFalse((bool)$result['ok']);
        $this->assertStringContainsString('no cam offers matched', strtolower((string)($settings['last_sync_message'] ?? '')));
        $samples = (array)($settings['last_sync_offer_name_samples'] ?? []);
        $this->assertSame('Generic Offer A', (string)($samples[0] ?? ''));
    }

    /**
     * Diagnostics should never contain raw API key values.
     *
     * @return void
     */
    public function test_sync_diagnostics_do_not_expose_api_key(): void {
        update_option(CrakRevenueCamManager::API_SETTINGS_OPTION, ['api_key' => 'super-secret-key']);
        $GLOBALS['_tmw_test_http_queue'] = [[
            'response' => ['code' => 200],
            'headers' => ['content-type' => 'application/json'],
            'body' => '{"echo":"api_key=super-secret-key","data":[]}',
        ]];

        CrakRevenueCamManager::sync_offers();
        $settings = get_option(CrakRevenueCamManager::API_SETTINGS_OPTION, []);
        $preview = (string)($settings['last_sync_body_preview'] ?? '');
        $this->assertStringNotContainsString('super-secret-key', $preview);
    }

    /**
     * Sync should prefer HTTPS endpoint when successful.
     *
     * @return void
     */
    public function test_sync_prefers_https_endpoint(): void {
        update_option(CrakRevenueCamManager::API_SETTINGS_OPTION, ['api_key' => 'secret-key']);
        $GLOBALS['_tmw_test_http_queue'] = [[
            'response' => ['code' => 200],
            'headers' => ['content-type' => 'application/json'],
            'body' => '{"data":[{"id":101,"name":"Camsoda - Revshare Lifetime","status":"Approved"}]}',
        ]];

        CrakRevenueCamManager::sync_offers();
        $settings = get_option(CrakRevenueCamManager::API_SETTINGS_OPTION, []);
        $this->assertStringStartsWith('https://', (string)($settings['last_sync_endpoint'] ?? ''));
    }
}
