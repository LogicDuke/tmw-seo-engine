...
- **Atomic `recover-manual-review`:** the review transition (skipped→stale), its review-audit row, and the `manual_review_recovered` fixture audit now commit in ONE outer cross-repository transaction. On fixture-audit failure everything rolls back — the review stays `skipped` with `execution_result=manual_assignment_preserved`, `stale_reason` unchanged, no recovery review-audit row, no fixture audit row — and the command exits non-zero with `recovery_fixture_audit_failed_rolled_back`.
- **Atomic `run-stale-validation` (execute mode):** the executor's stale transition, its review-audit row, and the `stale_validation_executed` fixture audit commit in ONE outer transaction. On fixture-audit failure the review stays `approved`/`not_executed` with no stale review-audit row, no fixture audit row, and no assignment write; non-zero exit with `stale_validation_fixture_audit_failed_rolled_back`. Dry-run remains transaction-free.
- **Safe transaction participation:** MySQL `START TRANSACTION` inside an open transaction implicitly commits, so `KeywordAssignmentReviewRepository` gains explicit external-transaction participation (`join_external_transaction()` / `leave_external_transaction()`, instance-scoped, cleared in `finally` on every service path): while enabled, the repository's own transaction verbs are no-ops and the validation orchestrator owns the single real boundary; failure semantics inside `update_with_audit` are unchanged. Participation is joined ONLY by the two validation units (statically asserted: no other production file joins), so sync, approve/reject, `mark_stale`, `mark_execution`, `execute-approved`, and every CLI command keep their existing per-operation atomicity — proven by a leak test in which a failed outer unit is followed by a normal audited transition that still rolls itself back via its own inner transaction.
- **Required refusal audit (option A):** execute-mode `recover_manual_review_refused` audits are mandatory — if the refusal-audit insert fails, the command exits non-zero with the distinct `refusal_audit_failed:<original-reason>` and never claims the refusal was audited.
- **Tests:** PR-F suites now 44 tests / 458 assertions — new rollback proofs for both units (review row byte-identical, audit counts unchanged on both sides, assignments untouched, ROLLBACK observed, successful retry after the fault clears), commit-together assertions on both success paths, the refusal-audit-failure case, and the participation-leak test; test doubles extended so the fixture fake's outer transaction snapshots/restores linked stores' audit rows and the review fake honors the participation gate. Focused PR-D/PR-E suites unchanged and green.

## 5.9.25-assignment-validation-v1.1.0 — 2026-07-27

PR-F revision after review: **production-validation tooling only** for the two PR-E guarantees not yet proven live — (A) a genuine manual assignment is preserved and reported as skipped by `execute-approved`, and (B) a real fresh-plan mismatch marks an approved review stale before any write. **No production cutover**: approval, Rank Math, generation, publishing, indexing, and ownership resolution are untouched; keyword candidate rows, Rank Math metadata, page content, postmeta, import evidence, and review snapshot hashes are never mutated by any action here; nothing runs on plugin load; every write requires an explicit CLI action, an explicit validation token, `--mode=execute` (dry-run default), and — for the stale and recovery workflows — explicit review/candidate IDs. There is no broad or unbounded command in this workflow.

**Review fixes over v1.0.0 (all six blocking/required findings):**

- **Opt-in override contract (blocking 1):** an active `stale_plan` fixture now influences NOTHING by itself. The automatic database loading of stale overrides inside ordinary `KeywordAssignmentMigrationService::analyze()` is fully removed (no stored, static, or global override state remains anywhere); overrides exist only as a strictly per-call `analyze( $filters, $validation_stale_overrides = [] )` argument that `KeywordAssignmentReviewExecutionService::execute_approved()` forwards verbatim. Ordinary migration commands, review sync, and execute-approved are byte-identical with active fixtures present (tested). Only the new explicit `run-stale-validation` action supplies overrides — after verifying the FULL validation context (exact token + exact review ID + exact candidate ID) against the ACTIVE fixture row; a wrong token, review ID, or candidate ID applies nothing and fails closed (tested per element).
- **`run-stale-validation` (new action):** verifies the context, re-checks the reviewed snapshot hash recorded at activation, re-proves the mismatch (and unchanged siblings) by simulation immediately before running, then drives the REAL executor scoped to exactly that one review with the override as a per-call argument — so `execute-approved` marks the review stale before any write, and the executor can never be reached in a state where it would write. Execute-mode runs append a `stale_validation_executed` audit event.
- **`recover-manual-review` (blocking 2, new action):** narrowly scoped, audited, idempotent recovery for the review stranded in `skipped` after fixture cleanup. Requires the exact token AND exact review ID; refuses while the fixture is still active; verifies the fixture's recorded source metadata; re-derives the review identity from the fixture's recorded assignment and refuses any other review (never resets arbitrary skipped rows); verifies the fixture assignment is gone and that no unrelated assignment reoccupied the identity; refuses reviews skipped for any other reason. Recovery itself uses the EXISTING workflow: the audited execution state machine moves `skipped -> stale` with reason `validation_manual_fixture_removed:<token>`, and the existing scoped sync restores `stale -> not_executed` once the fresh plan matches — after which the review approves/executes normally (full loop tested). Idempotent (`already_recovered` / `already_recovered_and_synced`); execute-mode refusals are audited as `recover_manual_review_refused`.
- **Fail-closed JSON (blocking 3):** `encode_values()` now returns null on failure and every write path hard-aborts on it — the transaction (including the assignment insert inside it) rolls back, the error propagates through the service, and the CLI exits non-zero. No fixture or audit row is ever persisted with fabricated `[]`. Tested for manual creation, stale creation, and lifecycle/audit persistence during restoration, plus a direct proof that the real encoder fails closed on invalid UTF-8.
- **Append-only fixture audit table (issue 4):** new `{prefix}tmw_keyword_assignment_validation_fixture_audit` (id, fixture_id, validation_token, fixture_type, action, old_state, new_state, actor, note, command_source, payload_hash of the normalized metadata snapshot, created_at). Creation, removal, restoration, stale-validation execution, manual-review recovery, and execute-mode refused recovery are all recorded; lifecycle transitions and their audit insertion commit atomically inside one transaction (audit failure rolls back the transition — tested); no update or delete path for audit rows exists anywhere (statically asserted).
- **Database-level concurrency (issue 5):** fixtures schema v2 adds two nullable active-identity columns with UNIQUE indexes — `active_token_key` (one active fixture per token) and `active_scope_key` (one active stale fixture per candidate scope / one active manual fixture per exact assignment identity), both cleared to NULL on the terminal transition. Duplicate/concurrent creates die deterministically at the index (`duplicate_active_fixture_identity`) even when every SELECT pre-check is blind (race-window tested). The runtime guard upgrades v1 installs via dbDelta with no backfill.
- **Sibling scope (issue 6):** `create-stale-fixture` now REFUSES activation (`sibling_plan_would_change_refusing_activation`) when the simulated override changes any fresh record other than the exact target review's — proven by comparing the candidate's full baseline vs simulated record maps, not merely reported. `run-stale-validation` repeats the same verification before every run. Tested with a shared-target-page candidate whose override would change a sibling pool's record.
- **Secondary-only manual fixtures (issue 7):** `FIXTURE_ROLES = ['secondary']`; `--role` values other than secondary (including primary) are rejected at the CLI layer AND at the service layer (`invalid_role_only_secondary_allowed`), both tested; runbook and examples updated.
- **Gated logging (issue 8):** every `error_log()` in the validation repository, service, and schema guard is gated behind `WP_DEBUG` or the dedicated `TMWSEO_KW_VALIDATION_DEBUG` constant (statically asserted: exactly one gated logging site per file). Normal production CLI output stays clean; failures surface through return values and non-zero CLI exits.

**Unchanged foundations:** additive-only schema (no FKs, no backfill, guard `ensure_keyword_assignment_validation_fixture_schema()` now verifies BOTH tables at version 2, CLI-invoked only, also wired into activation); unmistakable manual-fixture source metadata (`source_type=manual_validation_fixture`, `source_reference=validation:<token>`, ownership hard-disabled); overwrite/relabel refusals; token discipline; exact single-row cleanup with wrong-token and re-attribution refusals; fixture rows never deleted (history queryable after cleanup); dry-run default everywhere.

- **CLI:** `wp tmwseo keyword-assignment-validation <action>` with explicit actions only — `create-manual-fixture`, `inspect-manual-fixture`, `remove-manual-fixture`, `recover-manual-review`, `create-stale-fixture`, `run-stale-validation`, `restore-stale-fixture`, `status` — dry-run unless `--mode=execute`, non-zero exit on every refusal with nothing written.
- **Docs:** `docs/KEYWORD-ASSIGNMENT-VALIDATION-RUNBOOK.md` rewritten — opt-in override contract stated explicitly (ordinary commands ignore active fixtures; only `run-stale-validation` applies an override; no fixture may remain active after testing), 14-step manual-preservation flow incl. recovery, 10-step stale flow incl. the explicit run action, failure/refusal semantics, and the exact first production command (`status`).
- **Tests** (40 tests / 414 assertions in the two PR-F suites): behavioral suite runs the REAL migration/review/execution pipeline against transactional in-memory storage — opt-in contract (ordinary analyze/sync/execute byte-identical with active fixtures), exact-context application with per-element wrong-context refusals, sibling-change activation refusal, full recovery loop, recovery refusal matrix, JSON-encode and audit-insert rollback proofs (assignment writes included via linked-store transactions), unique-identity race refusals, plus all retained v1.0.0 coverage. Static suite adds audit-table DDL/append-only checks, v2 idempotency across both tables, per-call-override source proofs (no stored/static state, no fixture-repo access in the migration service), CLI action/role gates, and logging-gate assertions.

## 5.9.24-assignment-review-v1.0.0 — 2026-07-26

PR-E from the keyword cross-category ownership audit: reviewed, auditable, operator-controlled rollout workflow on top of the PR-D migration analyzer. **Review infrastructure only** — no production read or write path is switched to assignments; approval, rejection, generation, Rank Math sync, publishing, indexing, and model-generation behavior are untouched; candidate rows, Rank Math metadata, page content, and postmeta are never mutated by any review or execution phase; and the full 1,443-row migration plan is never auto-executed — every write requires an explicitly approved record plus an explicit `--mode=execute`.

- **Schema (additive only, no FKs):** `{prefix}tmw_keyword_assignment_review` — one row per deterministic planned-action identity (`UNIQUE review_key` = sha1 of migration version + candidate + pool + page_type + target_type + target_id + target_key), storing the reviewed snapshot (classification, role, planned status, canonical-owner flag, source attribution) pinned by `snapshot_hash`, review state (`pending|approved|rejected|deferred`), and execution state (`not_executed|executed|skipped|failed|stale`); `{prefix}tmw_keyword_assignment_review_audit` — append-only audit of every mutation (old/new states, actor, note, command source, snapshot hash). Runtime guard `ensure_keyword_assignment_review_schema()` (tag `[TMW-KW-ASSIGN-REVIEW-SCHEMA]`, version option `tmw_keyword_assignment_review_schema_version`) mirrors the PR-C guard for WP Pusher upgrades; both tables also wired into activation `create_or_update_tables()`.
- **`KeywordAssignmentReviewRepository`** (tag `[TMW-KW-ASSIGN-REVIEW]`): identity + snapshot hashing, whitelisted generic filtered listing with deterministic ordering, and fail-closed state machines. Review transitions: pending→approved/rejected/deferred; approved→rejected/deferred; deferred→approved/rejected; back to pending only via explicit `reset-to-pending`; rejected/deferred are never auto-converted; executed records are immutable. Approval is assignment-specific, refused for report-only records, non-approvable classifications, and stale records. Every change writes an audit row.
- **`KeywordAssignmentReviewSyncService`**: imports the CURRENT analyzer output (never external JSON) into the review queue — new identities pending, pending snapshots refreshed in place, human-reviewed records preserved when unchanged and marked stale when the plan changed, stale records restored when the fresh plan matches the reviewed snapshot again, executed records untouched, in-scope identities the analyzer no longer plans marked stale (skipped under a `--limit` truncated analysis), report-only classifications recorded only with explicit `--include-report-only`.
- **`KeywordAssignmentReviewExecutionService`**: dry-run by default; executes ONLY approved, non-stale, non-report-only `clear_primary_owner`/`secondary_assignment` records after per-record re-verification of the fresh planned action against the reviewed `snapshot_hash` (any difference → stale, not executed). `unused_owner` stays approvable for recording but is refused at execution (no state change) — legacy ownership is never bulk-activated. Writes go only through `KeywordAssignmentRepository`: manual assignments preserved (`skipped`), identical rows recorded as executed no-ops (idempotent re-execution), concurrent active-primary conflicts fail closed as `failed`; failed records are retryable, executed records never re-run (restartable).
- **`KeywordAssignmentReviewExportService`**: deterministic JSON/CSV export with a fixed 35-column whitelist; output paths restricted to `.json`/`.csv` (`.php` and everything else refused).
- **CLI:** `wp tmwseo keyword-assignment-review <action>` with explicit actions only (`sync`, `list`, `approve`, `reject`, `defer`, `reset-to-pending`, `export`, `execute-approved`, `status`) — no default action. Mutations require `--id=<ids>` or filters; filtered bulk mutation reports the matched count and requires `--confirm`; unbounded filtered mutation additionally requires `--all-matching`; per-record refusals are listed and exit non-zero. `execute-approved` is dry-run unless `--mode=execute`.

## 5.9.23-assignment-migration-v1.0.0 — 2026-07-26

PR-D from the keyword cross-category ownership audit: dry-run-first, idempotent migration/backfill infrastructure that analyzes current keyword evidence and can populate `tmw_keyword_assignments`. **Migration infrastructure only** — no production read or write path is switched to assignments; approval, rejection, generation, Rank Math sync, publishing, indexing, and model-generation behavior are untouched, and candidate rows, Rank Math metadata, content, and postmeta are never mutated by any migration phase.

- **`KeywordAssignmentMigrationAnalyzer`** (`includes/keywords/`, tag implied by service): pure deterministic classifier consuming PR-A ownership-report evidence rows (candidates, dual-linked import history, `rank_math_focus_keyword` metadata, content presence, legacy `_tmwseo_keyword` postmeta) plus existing assignment rows. Documented weighted evidence model (candidate owner 4, approval 3, Rank Math primary 4 / extra 2, content 2, import approved 2 / present 1, postmeta primary 3 / secondary 1); winner must strictly outscore the runner-up and clear a minimum bar — equal evidence yields `conflicting_owner` or `manual_review_required`, never a row-order winner. Classifications: `clear_primary_owner`, `secondary_assignment`, `stale_owner` (report-only, reassignment never automated), `unused_owner` (current ownership mirrored but flagged), `conflicting_owner`, `unresolved_target`, `manual_review_required`, `rejected_candidate_skipped`. Per-target import rejections stay per-target (planned as rejected secondaries). Existing manual assignments veto automated ownership on other targets; migration-owned primaries never oscillate when evidence shifts between runs; page-evidence collisions across target types force manual review.
- **`KeywordAssignmentMigrationService`** (tag `[TMW-KW-ASSIGN-MIGRATE]`): two explicit phases. `analyze()` (DEFAULT, read-only) streams evidence, produces a deterministic JSON-serializable report (version `kwmig-v1`, mode, filters, source counts, classification counts, per-decision evidence and reason codes, planned insert/update/unchanged/preserve actions, rollback tags). `execute()` re-derives the same model and writes only writable planned actions through `KeywordAssignmentRepository`: inserts fail closed against concurrent primaries; updates are partial upserts of only the changed migration-mutable fields, so unchanged repeats touch nothing (no duplicates, no timestamp churn); manual rows preserved untouched. `rollback()` (dry-run default) deletes only rows whose `source_type` is one of `migration_candidate|migration_import|migration_postmeta|migration_combined` AND `source_reference = kwmig-v1`, with a per-row double guard; restartable and idempotent.
- **Repository addition (no schema change):** `find_assignments_by_source()` read method; migration attribution uses the existing `source_type`/`source_reference`/`source_batch_id`/`source_import_row_id` columns.
- **CLI:** `wp tmwseo keyword-assignment-migration` with `--mode=dry-run|execute|rollback-dry-run|rollback-execute` (dry-run default; writes require the explicit execute modes), `--output=<file>` JSON report (`.php` refused), generic `--keyword/--candidate-id/--target-id/--pool/--limit` filters, non-zero exit on errors.
- **Tests** (35 new tests / 192 assertions): `KeywordAssignmentMigrationAnalyzerTest` covers the full classification matrix including order-independence of conflicts, duplicate-row collapse, cross-pool planning, colliding target IDs, manual-assignment vetoes, and idempotent update planning; `KeywordAssignmentMigrationServiceTest` proves read-only dry runs, idempotent re-execution, interrupted-run restartability, non-writable classifications never writing, manual preservation through execution and rollback, source-type-scoped rollback, candidate/Rank Math/content/postmeta immutability (behavioral + static), deterministic serialization, and — via a recursive source scan — zero production cutover. The PR-C sanctioned-files guard is deliberately extended with the two migration classes and the CLI.

## 5.9.22-assignment-schema-v1.0.0 — 2026-07-26

PR-C from the keyword cross-category ownership audit: adds the keyword assignment schema and repository layer, **additive only**. No production path reads or writes assignments yet — approval, rejection, candidate status, conflict decisions, generation, Rank Math, content, publishing, and indexing all remain on the candidate table unchanged. No backfill runs, no candidate row is touched, and the `UNIQUE KEY keyword` constraint on `tmw_keyword_candidates` is untouched.

- **New table `{prefix}tmw_keyword_assignments`** (`includes/db/class-schema.php`): separates keyword identity (candidate row) from per-target assignment. One row per assignment identity via a compact `UNIQUE assignment_key` — sha1 of (candidate, pool, page_type, target_type, target_id, target_key) — chosen over a multi-varchar composite unique key to stay inside utf8mb4 index-length limits; role and source batch are deliberately excluded from the identity so duplicate import rows and re-imports collapse onto one row. Primary-owner uniqueness is NOT an SQL constraint (MySQL/MariaDB lacks partial unique indexes; generated-column workarounds are dbDelta-incompatible) and is instead enforced transactionally in the repository. No foreign keys, matching every existing keyword table. Installer (`create_or_update_tables`) and a new WP Pusher-safe runtime guard (`ensure_keyword_assignments_schema`, option `tmw_keyword_assignments_schema_version`, tag `[TMW-KW-ASSIGN-SCHEMA]`) share one DDL source, so install/upgrade cannot drift and re-runs are idempotent.
- **New `KeywordAssignmentRepository`** (`includes/keywords/class-keyword-assignment-repository.php`, tag `[TMW-KW-ASSIGN]`): identity/lookup/count methods, validated `create_assignment`/`upsert_assignment` (identity fields immutable on upsert; ownership never flipped by upsert), assignment-scoped `update_assignment_status` (transition to blocked/rejected/inactive auto-clears `active_in_rank_math`), transactional fail-closed `set_primary_owner` (`START TRANSACTION` + `FOR UPDATE` + post-write single-owner verification + `ROLLBACK`, mirroring the batch repository's style), `clear_primary_owner`, and targeted single-row `delete_assignment` for future migration rollback. Built-in validator enforces role/status whitelists, canonical-owner-requires-primary, Rank-Math activation bans, and deterministic target identity (explicit `target_key` required when no numeric target exists). Loader-registered; zero production callers.
- **Ownership report extension** (read-only, backward-compatible key additions): summary now reports `assignments_table_present`, `assignment_count`, `candidates_with_assignments`, `candidates_with_multiple_assignments`, `primary_owner_violations`, `orphan_assignments`, `duplicate_assignment_identities` — all zeros/fail-safe while the table is absent or empty.
- **Tests** (26 new tests / 155 assertions): `KeywordAssignmentRepositoryTest` proves single-primary enforcement, primary switching, verification-failure rollback preserving the previous owner, identity dedupe across batches, cross-pool/two-target coexistence, candidate isolation (recording wpdb flags any candidate write), missing-table fail-safe, validator rules, and no hardcoding. `KeywordAssignmentSchemaStaticTest` proves DDL structural validity (no partial unique index, no generated columns, no FKs, index-safe widths), shared-DDL idempotency behaviorally (second guard run performs zero dbDelta calls), no destructive statements, no backfill and zero candidate contact during installation, and — via a full source scan — that only the five sanctioned files reference the assignment layer, so approval/rejection/generation/Rank Math provably do not.

## 5.9.21-scoped-reject-v1.0.0 — 2026-07-26

PR-B from the keyword cross-category ownership audit: manual import-row Reject is now scoped to the selected row only. **Data-integrity fix; no approval, generation, Rank Math, content, indexing, publishing, conflict-detection, schema, or Delete Batch behavior changed. No schema migration required; no existing candidate status is rewritten.**

- **Root cause removed.** The Reject branch of `KeywordPoolsAdminPage::handle_import_row_action()` previously demoted the globally unique candidate row (status → `ignored`). Because one candidate row per keyword can be referenced by rows in other batches, pools, targets, and page types, rejecting one review row could demote the keyword everywhere it is consumed. The Reject path now performs exactly one write — the selected import row (`status/result_action = rejected`, `result_reason = manually_rejected_row_only`, reviewer + timestamp via existing columns) — and intentionally never reads or writes the candidate table. The old demotion branch and its `manual_rejection_failed`/`candidate_status_update_failed` failure path are fully removed. Batch counts recalculate as before. Tag: `[TMW-KW-SCOPED-REJECT]` (logs row ID, batch ID, prior/new row status, candidate ID, and `candidate_mutation=skipped`; no payload contents).
- **Reviewer disclosure.** The batch review screen now states: "Rejecting an import row affects this review row only. It does not remove or demote the keyword from other targets or pools." Legacy `manually_rejected` rows continue to render unchanged.
- **Approval semantics untouched** (verified by static assertions). Candidate-level bulk moderation on the Keywords admin table (`tmwseo_kw_bulk_reject`) is a deliberate candidate-scope action on explicitly selected candidates and is intentionally out of scope.
- **Tests** (`tests/KeywordPoolScopedRejectTest.php`, 16 tests / 93 assertions): behavioral scoped-reject through the real repository against a state-tracking `$wpdb` (shared candidate across batches, two targets, both cross-pool directions, duplicate siblings, missing and stale candidate IDs, batch-count recalculation — each asserting the candidate table is neither written nor read), plus a marker-based regression guard that extracts the actual Reject branch from the handler source and fails if any candidate mutation, second reject path, unrelated write surface, or audit-example hardcoding reappears. `KeywordPoolImportHistoryStaticTest::test_manual_import_row_actions_are_post_nonce_and_candidate_safe` updated deliberately to pin the new scoped contract (it previously pinned the unsafe demotion and was already failing against drifted source).
