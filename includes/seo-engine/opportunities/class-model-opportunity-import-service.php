...
class ModelOpportunityImportService {
    ...
    public static function apply_rows(int $import_id, string $mode, array $rows, array $context = [], bool $preview = false): array {
        global $wpdb;
        $result = ['row_count'=>0,'created_count'=>0,'updated_count'=>0,'noise_count'=>0,'preview'=>[]];
        ...
        foreach ($rows as $row) {
            ...
            $model = (string)($context['model_entity'] ?? '');
            ...
            if ($existing) { $wpdb->update($opp_table, $payload, ['id'=>(int)$existing['id']]); $opp_id = (int)$existing['id']; $result['updated_count']++; }
            else { $payload['created_at']=current_time('mysql'); $wpdb->insert($opp_table,$payload); $opp_id = (int)$wpdb->insert_id; $result['created_count']++; }
