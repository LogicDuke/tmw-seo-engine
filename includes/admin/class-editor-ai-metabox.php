...
class Editor_AI_Metabox {
    ...
    public static function render($post): void {
        ...
        // ── Rollback button (shown only when a snapshot exists) ────────────
        if (\TMWSEO\Engine\Model\Rollback::has_snapshot((int)$post->ID)) {
            ...
            echo '<button '
                ...
                . 'class="button" '
                . 'style="width:100%;border-color:#c0392b;color:#c0392b;" '
                . 'data-post-id="' . esc_attr((string)$post->ID) . '" '
                . 'data-nonce="' . esc_attr(wp_create_nonce('tmwseo_rollback_' . $post->ID)) . '" '
                . 'data-ajax-url="' . esc_url(admin_url('admin-ajax.php')) . '"'
                . '>' . esc_html__('↩ Rollback to Pre-Generation', 'tmwseo') . '</button>';
            if ($snap_time) {
                echo '<span style="display:block;font-size:11px;opacity:.7;margin-top:2px">'
